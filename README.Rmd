...
You can install the development version of intercali from [GitHub](https://github.com/) with:
...
The `create_transect` function allows to create transects in the same way that the `make.design` from the `dssd` package. The `create_transect` function allows, among other things, to:
* choose different types of survey design such as zigzag or parallel transects
* choose the desired transect length (approximately)
* choose the angle of the transects
* choose the spacing between transects     
...
To calculate the probability that an individual could be observe thanks to the transects simulated, the first to do is to calculate distance between transects/segments and the observation of individuals. The function `calculate_distance` calculates the nearest transect/segment `transect_obj` for each simulated individual `obs_obj`. It returns an array with the name of the closest transect/segment for each individual and the distance in m and in km between them.
